...
\begin{itemize}
	...
	\item Periodic snapshotting is turned off. Instead, fault recovery is achieved by replaying the lost stream partitions from the latest buffered stream (possibly the source).\vspace{-3mm}
...
\subsection{Native Iterations on top of Dataflows}
...
\label{sec:batch-fault-tolerance}
\TODO{We need to simply say that we use the blocking data exchange to materialize intermediate results and redeploy in a spark fashion.}
